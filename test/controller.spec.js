...
describe("combineControllers", function() {
    ...
    describe("abort", function() {
        it("aborts all controllers", function() {
            ...
            const controller = combineControllers(ac1, ac2);
            controller.abort();
            expect(ac1.abort.callCount).to.equal(1, "First abort controller's 'abort' method should have been called");
            expect(ac2.abort.callCount).to.equal(1, "Second abort controller's 'abort' method should have been called");
            expect(ac1.signal.aborted).to.equal(true, "First abort controller should have been aborted");
            expect(ac2.signal.aborted).to.equal(true, "Second abort controller should have been aborted");
        });
    });
